            ...
            <div class="col-md-12">
                <div class="card">
                    <div class="card-header">
                        <h4 style="position: relative">Edit Cow
                            <a href="{{ route('admin.cow') }}" class="btn btn-primary float-end" style="position: absolute; right:0;">BACK</a>
                        </h4>
                    </div>

                    <div class="card-body">
                        <form action="{{ url('admin/cows/update/'.$cow->id) }}" method="POST" enctype="multipart/form-data">
                            @csrf
                            ...
                            <select name="sup_id" class="form-control">
                                @foreach($suppliers as $item)
                                    <option value="{{ $item->id }}" {{ $cow->sup_id == $item->id ? 'selected' : ''}}>
                                        {{ $item->sup_name }}
                                    </option>
                                @endforeach
                            </select>
                            ...
                            <div class="mb-3">
                                ...
                                <input type="text" name="cow_gene" class="form-control" value="{{ $cow->cow_gene }}">
                            </div>
                            @error('cow_gene')
                            <div class="my-2">
                            ...
                            <div class="mb-3">
                                ...
                                <input type="file" name="cow_img" class="form-control" accept=".png, .jpg, .jpeg" value="{{ $cow->cow_img }}">
                            </div>
                            @error('cow_img')
                                <div class="my-2">
                                    <span class="text-danger my-2">{{$message}}</span>
                            ...
                            @enderror
                            <input type="hidden" name="old_image" value="{{ $cow->cow_img}} ">
                                <div class="form-group">
                                    <img src="{{asset($cow->cow_img)}}" alt="" width="370px" height="370px">
                                </div>

                            <div class="mb-3">
                                <label for="Cow birth">Cow birth</label>
                                <input type="date" name="cow_birth" class="form-control" value="{{ $cow->cow_birth }}">
                            </div>
                            @error('cow_birth')
                            <div class="my-2">
                                <span class="text-danger my-2">{{$message}}</span>
