    ...
    <section class="content">
        ...
        <div class="container-fluid">
            <form action="" method="POST" id="cowForm" name="cowForm">
                                    ...
                                    <select name="supplier_id" id="supplier_id" class="form-control">
                                        ...
                                        @if ($suppliers->isNotEmpty())
                                            @foreach ($suppliers as $supplier)
                                                <option {{ $orderCow->supplier_id == $supplier->id ? 'selected' : '' }}
                ...
                $count = 0;
                @endphp
                @if (!empty($orderDetailsWithRelations))
                    @foreach ($orderDetailsWithRelations as $cow)
                            {{ $cow->cow_id }}
                                            ...
                                            <select name="cow_gene_id[]" id="cow_gene_id" class="form-control"
                                                ...
                                                @if ($cowGenes->isNotEmpty())
                                                    @foreach ($cowGenes as $cowGene)
                                                        <option {{ $cow->cow_gene_id == $cowGene->id ? 'selected' : '' }}
                                                            value="{{ $cowGene->id }}">{{ $cowGene->name }}</option>
                                                    @endforeach
                                                @endif
                                            </select>
                                            <p></p>
                                    ...
                                    <div class="col-md-4">
                                        <div class="mb-3">
                                            ...
                                            <input type="number" name="price[]" id="price" class="form-control"
                                                placeholder="Price" value="{{ $cow->price }}">
                                            <p></p>
                                        </div>
                                    </div>
                                    <div class="col-md-12">
                                        ...
                                        <div class="mb-3">
                                            <div class="form-group">
                                                ...
                                                <div class="form-check form-check-inline">
                                                    <input {{ $cow->gender == 'Man' ? 'checked' : '' }}
                                                        class="form-check-input" type="radio" name="gender[{{ $count }}]"
                                                        id="gender" value="Man">
                                                    <label class="form-check-label">Man</label>
                                                </div>
                                                <div class="form-check form-check-inline">
                                                    <input {{ $cow->gender == 'Woman' ? 'checked' : '' }}
                                                        class="form-check-input" type="radio" name="gender[{{ $count }}]"
                                                        id="gender" value="Woman">
...
@section('customJs')
    <script>
        ...
        $("#cowForm").submit(function(event) {
            ...
            $.ajax({
                url: '{{ route('admin.order-cows.update', $orderCow->id) }}',
                type: 'put',
